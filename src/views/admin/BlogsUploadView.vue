<template>
    <div>
        ...
        <div class=" relative grid-cols-1 w-4/5 mx-auto mt-8">
            ...
            <table class="table-fixed w-full">
                ...
                <tr class="text-gray-400 border-b-4 border-black my-4">
                    ...
                    <th class="w-1/6 text-xl">CreateTime</th>
                    <th class="w-1/12 text-xl">Views</th>
                    <th class="w-1/6 text-xl">Options</th>
                </tr>
                </thead>
                <tbody>
                <tr v-for="blog in blogs" :key="blog.id" v-if="blog.deleted===false" class="border-dashed border-b-2 border-gray-400 h-20 justify-center">
                    <td>{{blog.title}}</td>
                    <td class="text-center">{{blog.createTime}}</td>
...
export default {
    ...
    methods: {
        ...
        deleteBlog(blogId){
            axios.delete('/blogs/delete/'+blogId).then(response=>{
                alert('删除成功')
                this.$router.push('/blogsupload')
            }).catch(error=>{
                console.log(error);
            })
        },
